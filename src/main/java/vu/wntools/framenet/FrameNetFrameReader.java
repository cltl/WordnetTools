 ...
 */
public class FrameNetFrameReader extends DefaultHandler {
...
/*        ArrayList<String> descendants = new ArrayList<String>();
   ...
   public boolean frameNetConnected (String frame1, String frame2) {
        if (subToSuperFrame.containsKey(frame1)) {
            ArrayList<String> superFrames = subToSuperFrame.get(frame1);
            if (superFrames.contains(frame2)) {
                return true;
            }
            for (int k = 0; k < superFrames.size(); k++) {
                ...
                if (superToSubFrame.containsKey(frame)) {
                    ...
                    if (subFrames.contains(frame2)) {
                        return true;
                    }
                }
            }
        }
        if (superToSubFrame.containsKey(frame1)) {
            ArrayList<String> subFrames = superToSubFrame.get(frame1);
            if (subFrames.contains(frame2)) {
                return true;
            }
        }
   ...
   public ArrayList<String> getBridgingFrames (String frame1, String frame2) {
        ArrayList<String> matchedSuperFrames = new ArrayList<String>();
        if (subToSuperFrame.containsKey(frame1)) {
            ArrayList<String> superFrames = subToSuperFrame.get(frame1);
            if (superFrames.contains(frame2)) {
                if (!matchedSuperFrames.contains(frame2)) {
                    matchedSuperFrames.add(frame2);
                }
            }
            for (int k = 0; k < superFrames.size(); k++) {
                String frame = superFrames.get(k);
                if (superToSubFrame.containsKey(frame)) {
                    ArrayList<String> subFrames = superToSubFrame.get(frame);
                    if (subFrames.contains(frame2)) {
                        if (!matchedSuperFrames.contains(frame)) {
                            matchedSuperFrames.add(frame);
                        }
                    }
                }
            }
        ...
        if (superToSubFrame.containsKey(frame1)) {
            ArrayList<String> subFrames = superToSubFrame.get(frame1);
            if (subFrames.contains(frame2)) {
                if (!matchedSuperFrames.contains(frame1)) {
                    matchedSuperFrames.add(frame1);
                }
            }
        }
        return matchedSuperFrames;
    }
