 ...
 */
public class GetAverageSynsetDepthAndChildCount {
    ...
    static public void main (String[] args) {
        ...
        if (!pathToSynsetFile.isEmpty()) {
            ...
            for (int i = 0; i < synsets.size(); i++) {
                ...
                int childCount = wordnetData.getChildCountForId(s);
                System.out.println(s+"\t"+aDpeth+"\t"+childCount);
            }
        }
        else if (!synset.isEmpty()) {
                int aDpeth = wordnetData.getAverageDepthBySynset(synset);
                int childCount = wordnetData.getChildCountForId(synset);
                System.out.println(synset+"\t"+aDpeth+"\t"+childCount);
        }
    }
}
